...
def testEIF_Scoring_data(filename, number_of_trees, subsample_size, extensionLevel, dataset_type):
    ...
    if dataset_type == "10BIN":
        ...
        np_x_data = np.array(pd_data.iloc[:, :-1],dtype=float)
        ...
        np_x_data = np.array(data["X"],dtype=float)
    ...
    # old version of EIF with slightly modified computing score method
    # traing the forest
    ...
    random_seed_no = 53
    #new version of EIF
    forest_new_version = eif_new.iForest(training_data_without_label, ntrees=number_of_trees, sample_size=subsample_size_used, ExtensionLevel=extensionLevel,seed=random_seed_no)
    score_result = forest_new_version.compute_paths(X_in=training_data_without_label)
...
for dataset_name in dataset_names:
    ...
    for dataset_type in dataset_types:
        fpr, tpr, precision, recall, subsample_size_used = testEIF_Scoring_data(filename=dataset_name, number_of_trees=number_of_trees,
    ...
    plot_path_roc = "./plots/" + dataset_name + "-EIF-ROC_Curve-" + str(number_of_trees) + "-" + str(
        subsample_size_used) + "-" + extensionLevel_str + ".jpg"
    plot_path_prc = "./plots/" + dataset_name + "-EIF-PRC_Curve-" + str(number_of_trees) + "-" + str(
        subsample_size_used) + "-" + extensionLevel_str + ".jpg"
