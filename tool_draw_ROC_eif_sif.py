...
for dataset_name in dataset_names:
    ...
    fig, (ax_e, ax_s) = plt.subplots(ncols=2, nrows=1, figsize = (20,10))
    for dataset_type in dataset_types:
        fpr_e, tpr_e = draw_roc_from_EIF_result(filename=dataset_name, number_of_trees=number_of_trees,
                                                subsample_size=subsample_size,
                                                extensionLevel=extensionLevel,
                                                dataset_type=dataset_type)
        fpr_s, tpr_s = draw_roc_from_SIF_result(filename=dataset_name, number_of_trees=number_of_trees,
                                                subsample_size=subsample_size,
                                                extensionLevel=0,
                                                dataset_type=dataset_type)
    ...
    plot_path = "./plots/" + dataset_name + "-EIF-SIF-ROC_Curve-" + str(number_of_trees) + "-" + str(
        subsample_size) + "-" + extensionLevel_str + ".jpg"
    fig.savefig(plot_path)
    plt.tight_layout
    plt.close(fig)

    # y_predict = np.array(pd_data.loc[:, "Prediction"])
    # fpr_p, tpr_p, thresholds_p = skm.roc_curve(y_test, y_predict, pos_label=1)
    # print(filename+dataset_type)
    # print("pred")
    # print(skm.roc_auc_score(y_test,y_predict))
    # print("score")
    # print(skm.roc_auc_score(y_test, y_score))
    # print("fpr tpr pre")
    # print(skm.auc(fpr,tpr))
    # print("fpr tpr sco")
    # print(skm.auc(fpr_s,tpr_s))
    # return fpr_p, tpr_p, subsample_size
